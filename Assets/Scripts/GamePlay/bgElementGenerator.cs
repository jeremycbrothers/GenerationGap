...
public class bgElementGenerator
{
    ...
    private bool checkSingle(int x, int y)
    {
        if (currCell.getHasGroundTile()) //current cell is ground
        {
            ...
            if (nextCell.getCellType() == CellType.Empty) //cell above current cell is empty
            {
    ...
    private bool checkDouble(int x, int y)
    {
        ...
        {
            if(room.getCellAtCoord(x + 1, y).getHasGroundTile() && room.getCellAtCoord(x + 1, y + 1).getCellType() == CellType.Empty)
            {
                return true;
            }
        }
    ...
    private bool checkBush(int x, int y)
    {
            ...
            y + 2 < room.getCellsCols())
        {
            ...
            {
                if(room.getCellAtCoord(x - 1, y + 1).getCellType() == CellType.Empty && //enough empty space above ground
                    room.getCellAtCoord(x, y + 1).getCellType() == CellType.Empty &&
                    room.getCellAtCoord(x + 1, y + 1).getCellType() == CellType.Empty &&
                    room.getCellAtCoord(x - 1, y + 2).getCellType() == CellType.Empty &&
                    room.getCellAtCoord(x, y + 2).getCellType() == CellType.Empty &&
                    room.getCellAtCoord(x + 1, y + 2).getCellType() == CellType.Empty)
                {
                    Cell nextCell = room.getCellAtCoord(x, y + 1);
                    if(nextCell.getCellType() == CellType.Empty || nextCell.getCellType() == CellType.Enemy) //cell above current cell is empty
                    {
                        ...
                        placeElement(nextCell, position);
                    }
                    return true;
                }
            }
